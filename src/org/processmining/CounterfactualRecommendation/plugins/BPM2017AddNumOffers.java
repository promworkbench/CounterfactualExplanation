...
public class BPM2017AddNumOffers {

	@Plugin(

			name = "Add number of offers to BPM 2017 event log", parameterLabels = {"log BPI 2017", "log BPI 2017 offers"}, returnLabels = 
	...
	//			int num = 1;
				...
				for (Pair<XEvent, XTrace> et : offers.get(parts[1])) {
					// create a new trace
					...
					XAttributeDiscreteImpl numOffers = new XAttributeDiscreteImpl(labelTrace, offers.get(parts[1]).size());// num);
	...
	//				num++;
					...
					event.setAttributes(et.getValue().getAttributes());
