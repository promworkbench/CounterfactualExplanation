...
public class BPI2017rules {

	@Plugin(

			name = "BPM 2017 find rules of paper", parameterLabels = {"log BPI 2017", "log BPI 2017 offers"}, returnLabels = 
		{"Find the instances that fllow the rules of the paper"}, returnTypes = { XLog.class}, userAccessible = true, help = "help")
	...
	public static XLog applay(UIPluginContext context, XLog l1, XLog l2) {
    	...
    	for (XTrace trace : log) {
    		XAttributeMap amapTrace = trace.getAttributes();
    		double requestedAmount = Double.valueOf(amapTrace.get("RequestedAmount").toString());
    		String traceID = XConceptExtension.instance().extractName(trace);
    		String[] parts = traceID.toString().split("_");
    		String id = parts[1];
    		String applicationType = amapTrace.get("Applicationtype").toString();
    		int creditScore;
    		int numTerms;
    		boolean selected;
    		double firstWitthdra;
    		double monthlyCost;
    		for (XEvent event : trace) {
    			String actName = XConceptExtension.instance().extractName(event);
    			if (actName.equals("O_Create Offer")) {
